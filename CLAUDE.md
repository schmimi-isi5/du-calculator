...
- **Model selection (`domain/models.ts`) mirrors `QualityLevel` exactly** - a per-run pick, not a fixed setting, fixed on the `RequirementContext`/`ScoringResult` for their whole lifetime once chosen (`RequirementContext.model`/`ScoringResult.model`). Only meaningful when `AI_PROVIDER=anthropic`: `ANTHROPIC_SELECTABLE_MODELS` curates exactly which Anthropic models a run may pick (currently `claude-opus-5` and `claude-sonnet-5`) - deliberately not "any string the user types", since every listed model must accept the request shape `AnthropicProvider` always sends (adaptive thinking + the full `output_config.effort` range `QUALITY_PROFILES` uses); Haiku 4.5 and pre-4.7 models are excluded because they need a different thinking API shape and/or don't support `"xhigh"` effort. For every other `AI_PROVIDER` value there is nothing to choose - `selectableModelsFor()` returns the single model fixed by `AI_MODEL`/the provider default, and `POST /api/requirement-context` rejects an explicit `model` in the request body with a 400 in that case. The frontend's model picker (`RequirementPanel`, fetching `GET /api/ai-usage/models`) hides itself whenever there is only one option.
