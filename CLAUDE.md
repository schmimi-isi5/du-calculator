...
- "A missing piece of information is not automatically a question." The AI classifies every information gap (FACT/DERIVED/ASSUMPTION/CLARIFICATION_REQUIRED/UNKNOWN_NON_BLOCKING, see `domain/types.ts`); `scoring/clarificationGate.ts` - pure, deterministic, no LLM calls - decides which of those actually become a question (capped at `MAX_CLARIFICATIONS_PER_ROUND` per round, prioritized by potential DU impact, and no new question at all once `MAX_RESOLUTION_ROUNDS` is reached - the assessment finalizes on whatever's known instead of extending the dialog further). Assumptions are never presented as facts, always carry a confidence and a basis, and a user can CONFIRM/REJECT/EDIT one at any time (re-score explicitly afterward - editing an assumption does not auto-spend a new AI call).
- **Deliberately tuned for "assessment available quickly" over maximum thoroughness** (see `RepositoryContextBuilder.ts`'s budget comment): `AIProvider.assessRequirement` merges impact analysis and DU scoring into one AI call instead of two sequential ones; `AnthropicProvider`'s per-step effort (`EFFORT_BY_STEP`) runs repository analysis and context resolution at `"medium"`, keeping `"high"` only for the scoring call where nuanced judgment directly drives the DU result; and the repository context budget (`RepositoryContextBuilder.ts`) is a deliberate fraction of what would fit a model's context window. Revisit these constants together, not independently, if either speed or thoroughness needs to move.
