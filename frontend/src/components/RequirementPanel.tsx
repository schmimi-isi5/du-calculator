...
export function RequirementPanel({
  ...
  const currentModelLabel =
    model === null
      ? "…"
      : model === AUTO_OPTION_ID
        ? "Auto"
        : (usableModels.find((m) => m.id === model)?.displayName ?? model);
  ...
  return (
    <div className="card">
      ...
      <button type="button" className="repo-picker-toggle" onClick={() => setConfigExpanded((v) => !v)}>
        {configExpanded ? "▾" : "▸"} Qualitätsstufe: {QUALITY_LEVEL_META[qualityLevel].label} · Modell: {currentModelLabel}
        {privacyMode === "local-only" ? " · 🔒 Nur lokal" : ""}
      </button>

      {configExpanded && (
        <div className="repo-picker-body" style={{ marginBottom: 14 }}>
            ...
            <>
              ...
              {hiddenCount > 0 && (
                <p style={{ fontSize: 11, color: "var(--muted)", marginTop: 4 }}>
                  {hiddenCount} weitere{hiddenCount === 1 ? "s" : ""} Modell{hiddenCount === 1 ? "" : "e"} ohne
                  konfigurierten Zugang ausgeblendet - einrichtbar unter "Einstellungen".
                </p>
              )}
